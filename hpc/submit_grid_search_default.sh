...
mlp_default)
  ...
  N_HIDDEN_LAYERS_VALS=( 1 2 3 4 5 )
  HIDDEN_SIZE_VALS=( 0.5 1 2 )
  LOSS_VALS=( mse )
  ACTIVATION_VALS=( elu )
  L1_PENALTY_VALS=( 1e-4 1e-5 1e-6 )
  L2_PENALTY_VALS=( 1e-4 1e-5 1e-6 )
  ...
  VAR_ML_VALS=(
    ...
    cos_solar_zenith_angle,sw_albedo,skin_temperature,optical_depth_fl,cloud_fraction,temperature_fl,q
    cos_solar_zenith_angle,sw_albedo,skin_temperature,optical_depth_fl,cloud_fraction,temperature_fl,q,dz
  )
  ;;
...
for HIDDEN_SIZE in ${HIDDEN_SIZE_VALS[@]}; do
  ...
  singularity exec --containall \
    ...
    -B $TMPDIR:/tmp \
    $SIF_PATH \
    bash -c ". /miniconda/etc/profile.d/conda.sh && conda activate radiation && df -ah && cd $NB_DIR && \
