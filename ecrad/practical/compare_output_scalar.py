...
def main(input_srcfile, reference_output_srcfile, output_srcfiles, mode, dstdir):
    """
    Plot input files
    """
    ...
    styles = [{'lw':3, 'color':'#C375C5', 'ls':'-', 'zorder':10},
              {'lw':3, 'color':'#00C1AD', 'ls':'-'},
              {'lw':3, 'color':sns.color_palette()[2], 'ls':'-'},
              {'lw':2, 'color':sns.color_palette()[0], 'ls':'--'},
    ...
    dstfile = f"{dstdir}/{name_string}_{outputs_string}_vs_{reference_name_string}_scalar.svg"
    ...
    if mode == 'normal':
        ...
        eplt.compare_output_scalar_paper(input_srcfile, output_srcfiles, reference_output_srcfile, styles, dstfile=dstfile)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=['normal', 'paper'], default='normal')
    parser.add_argument("--dstdir", help="Destination directory for plots", default="./")
    args = parser.parse_args()
        
    main(args.input,  args.reference, args.outputs, args.mode, args.dstdir)
