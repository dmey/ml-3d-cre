...
def plot_inputs_scalars(IFS_srcfile, dstfile=None, include_t=False, line_ds='default'):
    with sns.plotting_context('notebook', font_scale=1.1), sns.axes_style('ticks'):
        ...
        fig, axes= plt.subplots(figsize=(25/2,2.25*nrows), nrows=nrows, ncols=ncols, gridspec_kw={'wspace':0.0, 'hspace':0.25}, squeeze=False)
        ...
        axes[i,j].set_xlabel('')
        axes[i,j].set_ylabel(r'$\cos (\theta_0)$')
        ...
        _ax0.set_yticks([0,0.5,1.0])
        _ax0.set_yticklabels([0,0.5,1.0],  color=sns.color_palette()[0])
        _ax0.set_ylabel(r'$\alpha$', color=sns.color_palette()[0])

        # https://matplotlib.org/3.3.1/gallery/ticks_and_spines/multiple_yaxis_with_spines.html
        if include_t:
            _ax1 = axes[i,j].twinx()
            _ax1.spines["right"].set_position(("axes", 1.1))
            make_patch_spines_invisible(_ax1)
            _ax1.spines["right"].set_visible(True)
            _ax1.yaxis.set_label_position("right")
            _ax1.yaxis.tick_right()
            _ax1.yaxis.label.set_color(sns.color_palette()[1])
            _ds.skin_temperature.plot(ax=_ax1, x='latitude', color=sns.color_palette()[1], lw=4, drawstyle=line_ds)
            [i.set_color(sns.color_palette()[1]) for i in _ax1.get_yticklabels()]
            _ax1.set_ylabel(r'$T_s$ in K', color=sns.color_palette()[1])
...
def plot_output(IFS_srcfile, ecRAD_srcfile, dstfile=None):
    ...
    with sns.plotting_context('notebook', font_scale=1.1), sns.axes_style('ticks'):
        ...
        has_cloud_radiative_effect = 'cloud_radiative_effect_lw' in _ds
        has_net_fluxes = 'flux_net_lw' in _ds
        has_up_dn_fluxes = 'flux_dn_lw' in _ds

        nrows=6
        if not has_cloud_radiative_effect:
            nrows -= 1
        if not has_net_fluxes:
            nrows -= 1
        if not has_up_dn_fluxes:
            nrows -= 2
        ...
        if has_net_fluxes:
            i+=1
            j=0
            irregular_pcolor(axes[i,j], _ds.latitude, _ds.pressure_hl, _ds.flux_net_lw, 
                                dict(cmap='Reds'),
                                cbar_kwargs={'pad':0.0125, 'aspect':10, 'label':'flux [W m$^{-2}$]'})
            axes[i,j].set_title("Net longwave flux")
            axes[i,j].set_xlabel('')

            j+=1
            irregular_pcolor(axes[i,j], _ds.latitude, _ds.pressure_hl, _ds.flux_net_sw, 
                                dict(cmap='Reds'),
                                cbar_kwargs={'pad':0.0125, 'aspect':10, 'label':'flux [W m$^{-2}$]'})
            axes[i,j].set_title("Net shortwave flux")
            axes[i,j].set_xlabel('')
...
def compare_output(IFS_srcfile, ctrl_srcfile, ecRAD_srcfile, dstfile=None):
    ...
    with sns.plotting_context('notebook', font_scale=1.1), sns.axes_style('ticks'):
        ...
        has_cloud_radiative_effect = all('cloud_radiative_effect_lw' in d for d in [ds, _ds])
        has_net_fluxes = all('flux_net_lw' in d for d in [ds, _ds])
        has_up_dn_fluxes = all('flux_dn_lw' in d for d in [ds, _ds])
        ...
        if has_net_fluxes:
            ...
            irregular_pcolor(axes[i,j], _ds.latitude, _ds.pressure_hl, da, 
                                dict(cmap='RdBu_r', vmin=vmin, vmax=vmax),
                                cbar_kwargs={'pad':0.0125, 'aspect':10, 'label':'flux [W m$^{-2}$]'})
            axes[i,j].set_title("Change to net longwave flux")
            axes[i,j].set_xlabel('')
            ...
            irregular_pcolor(axes[i,j], _ds.latitude, _ds.pressure_hl, da, 
                                dict(cmap='RdBu_r', vmin=vmin, vmax=vmax),
                                cbar_kwargs={'pad':0.0125, 'aspect':10, 'label':'flux [W m$^{-2}$]'})
            axes[i,j].set_title("Change to net shortwave flux")
            axes[i,j].set_xlabel('')
...
def compare_output2(IFS_srcfile, ctrl_srcfile, ecRAD1_srcfile, ecRAD2_srcfile, dstfile=None):
    
    ds = load_ecRAD(ctrl_srcfile, IFS_srcfile)
    
    with sns.plotting_context('notebook', font_scale=1.1), sns.axes_style('ticks'):

        _dss = [load_ecRAD(ecRAD1_srcfile, IFS_srcfile),
                load_ecRAD(ecRAD2_srcfile, IFS_srcfile)]

        nrows=6
        ncols=3

        fig, axes = plt.subplots(figsize=(35,2.5*nrows), nrows=nrows, ncols=ncols, sharex=True, sharey='row', gridspec_kw={'hspace':0.25, 'wspace':0.0})

        var_names = ['flux_dn_lw', 'flux_up_lw', 'flux_dn_sw', 'flux_up_sw',
                     'heating_rate_lw', 'heating_rate_sw']
        das = [{}, {}]
        vextents = {}
        for var_name in var_names:
            das[0][var_name] = _dss[0][var_name] - ds[var_name]
            das[1][var_name] = _dss[1][var_name] - ds[var_name]
            if var_name in ['heating_rate_lw', 'heating_rate_sw']:
                linear_pressure=True
                if linear_pressure:
                    vmin1, vmax1= get_vextents(das[0][var_name].where(_dss[0].pressure_fl > 100e2))
                else:
                    vmin1, vmax1= get_vextents(das[0][var_name])
                if linear_pressure:
                    vmin2, vmax2= get_vextents(das[1][var_name].where(_dss[1].pressure_fl > 100e2))
                else:
                    vmin2, vmax2= get_vextents(das[1][var_name])
            else:
                vmin1, vmax1 = get_vextents(das[0][var_name])
                vmin2, vmax2 = get_vextents(das[1][var_name])
            vextents[var_name] = (min(vmin1, vmin2), max(vmax1, vmax2))
        das = [_dss[0]] + das
        ...
        irregular_pcolor(axes[i,j], _ds.latitude, _ds.pressure_hl, da, 
                            dict(cmap='Reds', vmin=0, vmax=500),
                            cbar_kwargs={'pad':0.0125, 'aspect':10, 'label':'flux in W m$^{-2}$'})
        axes[i,j].set_title("Downwelling longwave flux")
        axes[i,j].set_xlabel('')
        ...
        #Heating rates
        cbar_kwargs = {'pad':0.0125, 'aspect':10, 'label':'$\dfrac{dT}{dt}$ in K d$^{-1}$'}
        i+=1

        linear_pressure=True

        da = das[j]['heating_rate_lw']
        if linear_pressure:
            vmin, vmax= get_vextents(da.where(_ds.pressure_fl > 100e2))
        else:
            vmin, vmax= get_vextents(da)
        ...
        for j, _ds in enumerate(_dss):
            j = j+1
            i = -1

            title_prefix = "3D signal:" if j == 1 else "3D prediction:"
            
            ### LW fluxes
            i+=1
            da = das[j]['flux_dn_lw']
            vmin, vmax = vextents['flux_dn_lw']
            irregular_pcolor(axes[i,j], _ds.latitude, _ds.pressure_hl, da, 
                                dict(cmap='RdBu_r', vmin=vmin, vmax=vmax),
                                cbar_kwargs={'pad':0.0125, 'aspect':10, 'label':'flux in W m$^{-2}$'})
            axes[i,j].set_title(f"{title_prefix} change to downwelling longwave flux")
            axes[i,j].set_xlabel('')

            i+=1
            da = das[j]['flux_up_lw']
            vmin, vmax = vextents['flux_up_lw']
            irregular_pcolor(axes[i,j], _ds.latitude, _ds.pressure_hl, da, 
                                dict(cmap='RdBu_r', vmin=vmin, vmax=vmax),
                                cbar_kwargs={'pad':0.0125, 'aspect':10, 'label':'flux in W m$^{-2}$'})
            axes[i,j].set_title(f"{title_prefix} change to upwelling longwave flux")
            axes[i,j].set_xlabel('')

            #SW fluxes
            i+=1
            da = das[j]['flux_dn_sw']
            vmin, vmax = vextents['flux_dn_sw']
            irregular_pcolor(axes[i,j], _ds.latitude, _ds.pressure_hl, da, 
                                dict(cmap='RdBu_r', vmin=vmin, vmax=vmax),
                                cbar_kwargs={'pad':0.0125, 'aspect':10, 'label':'flux in W m$^{-2}$'})
            axes[i,j].set_title(f"{title_prefix} change to downwelling shortwave flux")
            axes[i,j].set_xlabel('')

            i+=1
            da = das[j]['flux_up_sw']
            vmin, vmax = vextents['flux_up_sw']
            irregular_pcolor(axes[i,j], _ds.latitude, _ds.pressure_hl, da,
                                dict(cmap='RdBu_r', vmin=vmin, vmax=vmax),
                                cbar_kwargs={'pad':0.0125, 'aspect':10, 'label':'flux in W m$^{-2}$'})
            axes[i,j].set_title(f"{title_prefix} change to upwelling shortwave flux")
            axes[i,j].set_xlabel('')
            ...
            da = das[j]['heating_rate_lw']
            vmin, vmax = vextents['heating_rate_lw']
            irregular_pcolor(axes[i,j], _ds.latitude, _ds.pressure_fl, da, 
                            dict(cmap='RdBu_r', vmin=vmin, vmax=vmax),
                            cbar_kwargs=cbar_kwargs)
            axes[i,j].set_xlabel('')
            axes[i,j].set_title(f"{title_prefix} change to longwave heating rate (troposphere)")

            i+=1
            da = das[j]['heating_rate_sw']
            vmin, vmax = vextents['heating_rate_sw']
            irregular_pcolor(axes[i,j], _ds.latitude, _ds.pressure_fl, da,
                            dict(cmap='RdBu_r',vmin=vmin, vmax=vmax),
                            cbar_kwargs=cbar_kwargs)
            axes[i,j].set_xlabel('')
            axes[i,j].set_title(f"{title_prefix} change to shortwave heating rate (troposphere)")
...
def compare_output_scalar_paper(IFS_srcfile, ecRAD_srcfiles, reference_ecRAD_srcfile, ecRAD_styles, reference_label="", latitudes_to_highlight=None, line_ds='default', title=None, dstfile=None):
    ...
    with sns.axes_style("ticks", {"xtick.major.size": 8, "ytick.major.size": 8}):
        ...
        ds = load_ecRAD(reference_ecRAD_srcfile, IFS_srcfile)
        labels = ['3D signal', '3D prediction']

        for _style, _srcfile, label in zip(ecRAD_styles, ecRAD_srcfiles, labels):
            _ds = load_ecRAD(_srcfile, IFS_srcfile)

            #Longwave up flux at TOA
            (_ds.flux_up_lw - ds.flux_up_lw).isel(half_level=0).plot(ax=axes[0,0], x='latitude', drawstyle=line_ds, 
                                                          **{**_style, **{'label':label}})

            #Longwave down flux at surface
            (_ds.flux_dn_lw - ds.flux_dn_lw).isel(half_level=-1).plot(ax=axes[1,0], x='latitude', drawstyle=line_ds, 
                                                          **{**_style, **{'label':label}})

            #Shortwave up flux at TOA
            (_ds.flux_up_sw - ds.flux_up_sw).isel(half_level=0).plot(ax=axes[0,1], x='latitude', drawstyle=line_ds, 
                                                          **{**_style, **{'label':label}})

            #Shortwave down flux at surface
            (_ds.flux_dn_sw - ds.flux_dn_sw).isel(half_level=-1).plot(ax=axes[1,1], x='latitude', drawstyle=line_ds, 
                                                          **{**_style, **{'label':label}})

        #axes[0,0].set_ylabel('$\Delta$ flux in W m$^{-2}$')
        axes[0,0].set_ylabel('')
        axes[0,0].set_title('Change in upwelling\nlongwave flux at TOA')
        axes[0,0].set_xlabel('')
        axes[0,0].legend(loc='lower right', fontsize='x-small')

        #axes[1,0].set_ylabel('$\Delta$ flux in W m$^{-2}$')
        axes[1,0].set_ylabel('')
        axes[1,0].set_title('Change in downwelling\nlongwave flux at surface')
        axes[1,0].set_xlabel('')

        axes[0,1].set_ylabel('$\Delta$ flux in W m$^{-2}$')
        axes[0,1].set_title('Change in upwelling\nshortwave flux at TOA')
        axes[0,1].set_xlabel('')

        

        axes[1,1].set_ylabel('$\Delta$ flux in W m$^{-2}$')
        axes[1,1].set_title('Change in downwelling\nshortwave flux at surface')
        axes[1,1].set_xlabel('')

        axes[-1,0].set_xlim(-90,90)
        axes[-1,0].set_xticks(np.arange(-90,90,30))
        format_latitude(axes[-1,0])
        axes[-1,0].set_xlabel('Latitude')
...
def plot_output_profile(latitude, IFS_srcfile, ecRAD_srcfiles, linedicts, dstfile=None, 
    ...
    with sns.plotting_context('talk'):
        ...
        for j, ecRAD_srcfile in enumerate(ecRAD_srcfiles):
            ...
            has_clear_sky = 'flux_up_lw_clear' in _ds
            has_cloud_radiative_effect = 'cloud_radiative_effect_lw' in _ds
            has_up_dn_fluxes = 'flux_dn_lw' in _ds
